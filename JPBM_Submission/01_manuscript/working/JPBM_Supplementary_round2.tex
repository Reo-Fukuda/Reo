...
% TODO(construct-validity): OUTSTANDING BEFORE SUBMISSION — requires item-level
% data, not new prose. Reviewer (2026-08-27) asks for discriminant validity
% between measures collected at the same time from the same respondents with
% similarly negative wording: Loss of Product Luxuriousness vs Loss of Brand
% Luxuriousness (Study 2), and Failure Inference vs Loss of Brand Luxuriousness
% (Studies 3a-3c). Add to this section: (1) two-factor vs one-factor CFA,
% (2) inter-factor correlations, (3) AVE or HTMT, (4) per-study means, SDs and
% correlation matrix. Also consider renaming Failure Inference to Perceived
% Launch Failure or Launch-Failure Judgment, which describes the items more
% directly; if the current name is kept, sharpen the first-mention definition to
% say it measures the degree to which the launch itself is judged a failure
% given the displayed outcome, not an inference about the cause of failure.
\SIMajorHeading{H-4. X Data: Retrieval Pipeline and Audit Status}
