...
Study 2 also measured Inferred Cost as a check on the product-form difference. A three-factor model including it fitted well ($\chi^2$(24) = 45.08, CFI = .990, TLI = .985, RMSEA = .057, SRMR = .036; $\alpha$ = .907), with Inferred Cost correlating $\varphi$ = -.615 with Loss of Product Luxuriousness and $\varphi$ = -.383 with Loss of Brand Luxuriousness.
...
Each experiment embedded one or more instructed-response items, and a participant who failed any of the checks administered in that study was excluded before analysis. Applying every check administered rather than only one of them keeps the exclusion rule constant across studies. The checks and the resulting analytic samples were as follows.
