...
\noindent The stimulus images appear in Figure A1.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.49\linewidth]{figures/figure_A1a_study2_physical.png}\includegraphics[width=0.47\linewidth]{figures/figure_A1b_study2_digital.png}
...
\noindent The displayed-information screen appears in Figure A2.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/figure_A2_study3a_displayed_information.png}
...
\noindent The two images appear in Figure A3.

\begin{figure}[H]
\centering
\includegraphics[width=0.39\linewidth]{figures/figure_A3a_study3b_central_logo.png}\includegraphics[width=0.56\linewidth]{figures/figure_A3b_study3b_bee_image.png}
...
The main text theorizes that initial price and distribution information changes the interpretive context in which displayed buyer and market-value information is read. One exploratory item, ``I believe this NFT is a product intended to generate revenue for the company'' (7-point scale), is reported for Study 3a. This analysis was not preregistered and is exploratory; the item is a single author-developed item, not a validated construct measure.

As reported in the main text, the purchased/comparable condition (\emph{M} = 4.74, \emph{SD} = 1.32, \emph{n} = 70) scored higher on this item than the complimentary magazine-gift condition (\emph{M} = 3.86, \emph{SD} = 1.68, \emph{n} = 65; Welch's \emph{t}(121.33) = 3.38, \emph{p} = .001, \emph{d} = 0.59).
...
\noindent \textbf{Model 1: Predicting the single monetization item (M1).} The complimentary condition significantly reduced scores on the item ($a_1$ = -0.88, \emph{SE} = 0.26, \emph{t}(133) = -3.41, \emph{p} = .001, 95\% CI {[}-1.39, -0.37{]}; \emph{R}\textsuperscript{2} = .08).

\noindent \textbf{Model 2: Predicting Failure Inference (M2).} Both the direct effect of condition ($a_2$ = -0.93, \emph{SE} = 0.23, \emph{t}(132) = -4.05, \emph{p} \textless{} .001) and the effect of the monetization item ($d_{21}$ = 0.27, \emph{SE} = 0.07, \emph{t}(132) = 3.70, \emph{p} \textless{} .001) were significant (\emph{R}\textsuperscript{2} = .24).

\noindent \textbf{Model 3: Predicting Loss of Brand Luxuriousness (Y).} Failure Inference significantly increased Loss of Brand Luxuriousness ($b_2$ = 0.65, \emph{SE} = 0.09, \emph{t}(131) = 7.54, \emph{p} \textless{} .001). The effect of the monetization item was also significant ($b_1$ = -0.17, \emph{SE} = 0.08, \emph{t}(131) = -2.19, \emph{p} = .030). The direct effect of condition was not significant ($c'$ = 0.07, \emph{SE} = 0.24, \emph{t}(131) = 0.31, \emph{p} = .755; \emph{R}\textsuperscript{2} = .32).
...
This section reports the brand classification used in the X data analysis. The classification was fixed prior to the exploratory analysis and used to estimate the interaction with the Free/Paid classification. The full classification appears in Table E1.
\setcounter{table}{0}
...
Each post was scored with VADER sentiment analysis (Hutto and Gilbert, 2014), implemented in a manner consistent with prior online-review research (Costa \emph{et al.}, 2019). Estimation proceeded in two tracks: (1) a linear mixed model (LMM) with brand as a random intercept and calendar time as a covariate, and (2) a generalized estimating equations (GEE) model with negative posts (compound $\leq$ -0.05) as the outcome. The Free/Paid classification was coded as paid (0 = Free, 1 = Paid), and brand type was coded as luxury (0 = Non-Luxury, 1 = Luxury). GEE yields standard errors that are robust to within-brand correlation in the post-level data.
...
\noindent Figure G1 plots the three time points by condition.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/figure_G1_study4.pdf}
...
Each model was estimated by maximum likelihood on that study's analytic sample (defined by the instructed-response checks listed in H-5), with factor variances fixed at unity and each item free to load only on its assigned factor. Because these are six-indicator models with eight degrees of freedom, we report CFI, TLI and SRMR alongside RMSEA: in models with few degrees of freedom RMSEA tends to be inflated and to reject correctly specified models, so it is best read together with the other indices rather than on its own (Kenny \emph{et al.}, 2015). Table H1 compares the fit of the hypothesized two-factor model with a one-factor alternative in which all six items load on a single factor.
...
\midrule
Study 2 (\emph{N} = 276) & Two-factor & 12.32 & 8 & .997 & .994 & .044 & .016 \\
 & One-factor & 190.46\textsuperscript{*} & 9 & .873 & .788 & .271 & .109 \\
\addlinespace
Study 3a (\emph{N} = 135) & Two-factor & 13.02 & 8 & .994 & .988 & .068 & .039 \\
 & One-factor & 252.33\textsuperscript{*} & 9 & .687 & .479 & .449 & .191 \\
\addlinespace
...
\midrule
Study 2 & Loss of Product Luxuriousness & .742--.904 & .848 & .855 & .664 & .815 \\
 & Loss of Brand Luxuriousness & .933--.947 & .958 & .958 & .885 & .941 \\
 & \multicolumn{6}{l}{\emph{Factor correlation} $\varphi$ = .707; HTMT = .721} \\
\addlinespace
Study 3a & Failure Inference & .816--.964 & .918 & .923 & .801 & .895 \\
 & Loss of Brand Luxuriousness & .887--.962 & .949 & .951 & .867 & .931 \\
 & \multicolumn{6}{l}{\emph{Factor correlation} $\varphi$ = .545; HTMT = .584} \\
\addlinespace
...
\bottomrule
\end{tabular}
\raggedright\emph{Note.} $\lambda$ = standardized loading; CR = composite reliability; AVE = average variance extracted; HTMT = heterotrait--monotrait ratio of correlations. Every standardized loading was .697 or higher; the smallest ratio of an unstandardized loading to its asymptotic standard error was 11.35, so all loadings were significant at \emph{p} \textless{} .001.
\end{singlespace}
\end{table}

Three indices point the same way. The latent factor correlations ranged from .543 to .707: substantial, as one would expect of judgments formed in the same session about the same offering, but well short of unity. Average variance extracted exceeded .50 for every construct (range .664 to .885), and in each study the square root of the average variance extracted for both constructs exceeded their correlation, satisfying the Fornell and Larcker (1981) criterion. The heterotrait--monotrait ratio ranged from .565 to .728, below the .85 threshold recommended by Henseler \emph{et al.} (2015) in every study. Table H2 reports these indices by study.

Two qualifications belong with these results. First, RMSEA for the two-factor models ranged from .044 to .126, exceeding .08 in Studies 3b and 3c even though CFI, TLI and SRMR indicated close fit in the same models. This pattern is characteristic of models with few degrees of freedom rather than an indication of a mis-specified factor structure, and we report it rather than selecting indices that favor the model. Second, the evidence here concerns discriminant validity between the measures as scored; it does not establish that Failure Inference captures a causal attribution, and we do not claim that it does (Section H-2 and the main-text limitations).

Study 2 also measured Inferred Cost as a check on the product-form difference. A three-factor model including it fitted well ($\chi^2$(24) = 45.08, CFI = .990, TLI = .985, RMSEA = .057, SRMR = .036; $\alpha$ = .907), with Inferred Cost correlating $\varphi$ = -.615 with Loss of Product Luxuriousness and $\varphi$ = -.383 with Loss of Brand Luxuriousness. Item-level means, standard deviations and correlations for each study appear in Table H3.
...
\midrule
Study 2 & LPL1 & 3.49 & 1.70 & 1.000 &  &  &  &  &  \\
 & LPL2 & 3.37 & 1.73 & .716 & 1.000 &  &  &  &  \\
 & LPL3 & 3.57 & 1.75 & .558 & .679 & 1.000 &  &  &  \\
 & LBL1 & 3.39 & 1.75 & .530 & .566 & .515 & 1.000 &  &  \\
 & LBL2 & 3.20 & 1.66 & .543 & .581 & .502 & .882 & 1.000 &  \\
 & LBL3 & 3.46 & 1.71 & .569 & .614 & .504 & .891 & .881 & 1.000 \\
\addlinespace
Study 3a & FI1 & 4.59 & 1.48 & 1.000 &  &  &  &  &  \\
 & FI2 & 4.69 & 1.59 & .868 & 1.000 &  &  &  &  \\
 & FI3 & 4.41 & 1.64 & .719 & .789 & 1.000 &  &  &  \\
 & LBL1 & 4.44 & 1.49 & .483 & .489 & .479 & 1.000 &  &  \\
 & LBL2 & 4.49 & 1.62 & .542 & .527 & .499 & .849 & 1.000 &  \\
 & LBL3 & 4.51 & 1.64 & .452 & .445 & .433 & .910 & .836 & 1.000 \\
\addlinespace
...
\SIMajorHeading{H-5. Instructed-Response Checks and Exclusions}

Each experiment embedded one or more instructed-response items, and a participant who failed any of the checks administered in that study was excluded before analysis. Applying every check administered rather than only one of them keeps the exclusion rule constant across studies. The checks and the resulting analytic samples appear in Table H4.

\begin{table}[H]
\centering
\begin{singlespace}
\caption{Instructed-Response Checks and Analytic Samples}
\small
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{0.42\columnwidth}rrr@{}}
\toprule
Study & Checks administered (correct response) & Recruited & Excluded & Analytic \emph{N} \\
\midrule
Study 1 & One check (7) & 674 & 4 & 670 \\
Study 2 & Three checks (1; 6; 6) & 283 & 7 & 276 \\
Study 3a & Two checks (1; 6) & 140 & 5 & 135 \\
Study 3b & Two checks (1; 6) & 282 & 1 & 281 \\
Study 3c & One check (1) & 287 & 5 & 282 \\
Study 4 (SI) & One check (1) plus the preregistered ceiling criterion & 282 & 9 & 273 \\
\bottomrule
\end{tabular}
\raggedright\emph{Note.} Exclusions are counts of distinct participants; a participant failing more than one check is counted once. In Study 3b every participant passed the second check, so the analytic sample is the same under either rule. The Study 4 ceiling criterion, specified in the preregistration, excluded three participants who scored at the scale ceiling (10) at both \emph{T}0 and \emph{T}1. Total analytic \emph{N} across the six experiments = 1,917.
\end{singlespace}
\end{table}

\SIMajorHeading{H-6. X Data: Retrieval Pipeline and Audit Status}
...
\noindent The cleaning pipeline and the counts it removed at each step appear in Table H5.
...
Kenny, D.A., Kaniskan, B. and McCoach, D.B. (2015), ``The performance of RMSEA in models with small degrees of freedom'', \emph{Sociological Methods \& Research}, Vol. 44 No. 3, pp. 486--507, doi: 10.1177/0049124114543236.
